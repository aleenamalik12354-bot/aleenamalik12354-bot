...
const Experience = () => {
  ...
  return (
    <section
      id="experience"
      className="py-24 pb-24 px-[12vw] md:px-[7vw] lg:px-[16vw] font-sans bg-transparent clip-path-custom-2 overflow-hidden"
    >
      {/* Section Title */}
      <motion.div
      ...
      >
        <h2 className="text-4xl font-bold text-[var(--text-primary)]">{t('experience.title')}</h2>
      ...
      >
        ...
        {experiences.map((experience, index) => (
          ...
          >
            ...
            {/* Content Section */}
            <div
              className={`w-full sm:max-w-md p-6 sm:p-8 rounded-2xl border-2 border-[#8245ec]/50 hover:border-[#8245ec] bg-[var(--card-bg)] backdrop-blur-lg shadow-[0_0_15px_rgba(130,69,236,0.15)] hover:shadow-[0_0_30px_rgba(130,69,236,0.4)] transition-all duration-300 group ${index % 2 === 0 ? "sm:mr-auto sm:text-right" : "sm:ml-auto sm:text-left"
            ...
            >
              ...
              <div className={`flex items-center gap-4 mb-4 ${index % 2 === 0 ? "sm:flex-row-reverse" : "sm:flex-row"}`}>
                ...
                <div className="flex flex-col">
                  <h3 className="text-xl sm:text-2xl font-bold text-[var(--text-primary)] group-hover:text-[#8245ec] transition-colors duration-300">
                    {experience.role}
                  </h3>
                  <h4 className="text-sm sm:text-base text-[var(--text-secondary)] font-medium">
                    {experience.company}
                  </h4>
                </div>
              </div>
              ...
              <div className={`flex flex-wrap gap-2 ${index % 2 === 0 ? "sm:justify-end" : "sm:justify-start"}`}>
                {experience.skills.map((skill, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1 text-xs font-medium text-[var(--accent-color)] bg-purple-500/10 border border-purple-500/20 rounded-full"
                  >
                    {skill}
                  </span>
                ))}
              ...
              <div className={`mt-6 ${index % 2 === 0 ? "sm:text-right" : "sm:text-left"}`}>
                <a
                  ...
                  className="inline-flex items-center gap-2 text-[var(--text-secondary)] hover:text-[#8245ec] transition-colors duration-300 text-sm font-medium"
                >
